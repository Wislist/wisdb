/*
	transaction.go 实现了sm内部的transaction结构，该结构内保存了sm中事务需要的必要信息
*/

package sm

import "mydb/src/main/backend/tm"

type transaction struct {
	XID          tm.XID     
	Level        int			    // 隔离度
	snapshot     map[tm.XID]bool    // 事务快照
	Err          error              // 发生错误，该事务只能被回滚
	AutoAbortted bool               // 是否自动回滚
}
