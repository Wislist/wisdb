/**
 * serializability_manager.go 保障了调度的可串行化，同时实现了MVCC
 * 当事务发生ErrCannotSR错误时，SM会对该事务进行回滚
 */
package sm
...
import (
	...
	"mydb/src/main/backend/sm/locktable"
...
type serializabilityManager struct {
	...
	lt locktable.LockTable // lock table cache
...
func (sm *serializabilityManager) abort(xid tm.XID, auto bool) {
	...
	t := sm.tc[xid]
	if auto == false {
		delete(sm.tc, xid) // 如果自动撤销, 不完全注销该事务, 只是潜在的将其回滚; 如果是手动, 则彻底注销.
	}
	sm.lock.Unlock()
