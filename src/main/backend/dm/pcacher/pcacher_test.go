...
func TestPcacherOpenAndTruncate(t *testing.T) {
	...
	p2, _ := Open(base, PAGE_SIZE*20)
	if p2.NoPages() != 2 {
		t.Fatalf("NoPages=%d, want=2", p2.NoPages())
	}

	_ = p2.TruncateByPgno(1)
	if p2.NoPages() != 1 {
...
// TestPcacherCreateMemTooSmallError 验证内存不足时会返回错误。
func TestPcacherCreateMemTooSmallError(t *testing.T) {
	base := filepath.Join(t.TempDir(), "pcacher_small_mem")
	_, err := Create(base, PAGE_SIZE*5)
	if err == nil {
		t.Fatal("expected error for too-small memory, got nil")
	}
	if !errors.Is(err, ErrMemTooSmall) {
		t.Fatalf("error=%v, want=%v", err, ErrMemTooSmall)
	}
}
